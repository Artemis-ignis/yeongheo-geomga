...
- `.codex/generated_images`와 `artifacts/2d-build`의 파일·SHA-256은 로컬 출력 보관·동일성 증거일 뿐, 어느 OpenAI 계정/관할/서비스 약관으로 생성됐는지, Input 권리가 있었는지, 팀 대표자가 제출·홍보권을 갖는지는 증명하지 않는다. 그러므로 위 OpenAI 약관을 근거로 73개 중 어느 것도 `own` 또는 권리 확인으로 승격하지 않는다.
...
| 확인 항목 | 현재 관찰값 | 증거 | 판정 |
|---|---:|---|---|
| 매니페스트 자산 | 117개 | `tools/asset-manifest.json` | 기술 목록 확인 |
| 실제 `public/assets` 파일 | 117개 | `public/assets/**` 재귀 목록 | 매니페스트와 수량 일치 |
| 현재 제출 runtime allowlist | 73개 | `tools/submission-assets.mjs`의 `SUBMISSION_RUNTIME_ASSETS` | 아래 73개만 계획상 번들 포함 |
| allowlist 밖 public 자산 | 44개 | 매니페스트와 allowlist 차집합 | 제출 번들에서 제외하도록 설계됨. generic `public/**` 배포로 재포함되지 않는지 별도 확인 필요 |
| 권리 증거 파일 | 법적 권리 증거 없음; 기술 provenance는 73/73 추적 | 직접 출력 8/73, sprite source chain 14/73(원본과 source exact 11 + 원본·생성 로그·변환 재현 3), release-v3 UI atlas chain 19/73, release-v5 의미별 아이콘 atlas chain 32/73, exact original 미검출 0/73. 계정·Input 권리·대표자 권한·대회 공개 허락 기록은 없음 | 기술 provenance 73/73 확인; 법적 권리 0/73·차단 |
...
아래 73개는 현재 `tools/submission-assets.mjs` allowlist와 매니페스트의 교집합이다. 전부 기술 provenance는 `generated`로 분류한다. 상태 문구의 `로컬 원본 확인` 또는 `source chain 확인`은 파일 동일성과 경로가 확인됐다는 뜻일 뿐, 권리 상태가 `own` 또는 제출 승인으로 바뀌었다는 뜻이 아니다. `외부 파이프라인` 표시는 출력 파일의 외부 사용권이 확인됐다는 뜻이 아니다.
...
| 경로 | 확인된 역할 | 현재 증거/한계 |
|---|---|---|
| `tools/asset-manifest.json` | 117개 자산의 `path`, `source`, `consumer`, 용량 제한을 선언 | 기술 목록만 증명하며 생성 계정·Input 권리·출력 약관·정확한 명령 실행은 기록하지 않음; 현재 작업 트리에서 수정 상태 |
...
현재 저장소에는 73개 결과 각각의 계정·Input 권리·대표자 공개 권한을 고정한 법적 증거가 없다. 그러므로 `sourceUrl`, atlas crop, exact SHA match를 “로컬 provenance 부분 확인”까지만 사용하고 “권리 승인”으로 확장하지 않는다.

#### 프로젝트 커밋·문서 연결

- snapshot의 현재 HEAD는 `351541c8acf64029d27c016612b0383e0c79a74d` (`Integrate ImageGen v3 Forge enemy assets`)이며 작업 트리는 dirty다. `tools/asset-manifest.json`은 수정 상태이고, 14개 runtime sprite와 AS-02/AS-21/AS-73은 현재 untracked라 제출 기준 커밋이 없다.
- tracked direct output의 최근 커밋은 AS-01 `fd84f6c`, AS-17 `6877dfc`, AS-18 `0d3027e`, AS-19 `e43ee89`, AS-20 `05bdd8a`다. AS-02, AS-21, AS-73은 파일 fingerprint만 있고 프로젝트 커밋은 없다. 14개 sprite runtime output은 모두 working tree/untracked다.
...
| 분류 | 개수 | 의미 |
|---|---:|---|
| `generated` | 73 | 매니페스트가 ImageGen 또는 ImageGen 후처리 provenance를 명시 |
| `own` | 0 | 저장소에서 권리 주체를 확인하는 명시적 소유·양도 증거를 찾지 못함 |
| `external` | 0개의 직접 제출 파일 | 직접 외부 원본 파일은 allowlist에 없지만, 생성 서비스·도구 의존성은 아래 별도 게이트 대상 |
| `unknown` | 0개의 기술 원천 | 기술 source 라벨은 있으나 **권리 상태는 73개 전부 미확인** |
| 직접 제출 파일과 `.codex/generated_images` exact match | 8/73 | AS-01, AS-02, AS-17~AS-21, AS-73의 제출 파일 자체가 local generated original과 SHA-256 일치 |
| sprite source와 `.codex/generated_images` exact match | 11/73 | AS-03, AS-05~AS-13, AS-15의 source sheet가 local generated original과 SHA-256 일치; runtime은 파생물 |
| 원본·생성 이벤트·변환 명령으로 재현한 sprite chain | 3/73 | AS-04, AS-14, AS-16의 source와 runtime을 격리 재생성하여 파일 SHA와 RGBA pixel 모두 exact match |
| release-v3 UI atlas source chain | 19/73 | AS-22~AS-40은 로컬 ImageGen atlas와 결정적 crop 산출물로 연결됨 |
| release-v5 의미별 아이콘 atlas source chain | 32/73 | AS-41~AS-72는 두 로컬 ImageGen atlas와 결정적 256×256 crop 산출물로 연결됨 |
| source는 있으나 exact original 미검출 | 0/73 | 기존 AS-04, AS-14, AS-16 원본과 변환 chain을 복구·재현함 |
| 기술 provenance chain 확인 | 73/73 | 8 direct + 11 exact sprite source + 3 reproduced sprite chain + 19 legacy UI atlas + 32 semantic icon atlas |
| OpenAI 약관의 조건부 Output 근거 후보 | 73/73 | 로컬 생성 chain이 연결된 범위일 뿐 계정·Input·대회 권리 clearance 아님 |
| 법적 권리 증거 확인 | 0/73 | OpenAI 약관의 일반적 Output 조항, 로컬 SHA, QA PASS는 자산별 권리·대회 제출 허락을 증명하지 않음 |

이 표의 `own=0`, `unknown=0`은 기술 source 라벨의 분류이며 안전 판정이 아니다. provenance 연결 수는 권리 승인 수가 아니다. 계정·Input·인물/상표·팀/대표자 권한·대회 공개 허락이 확인되지 않은 상태이므로 제출 가능 수와 최종 권리 확인 수는 현재 0/73으로 취급한다.

## allowlist 밖 44개와 외부 파이프라인 위험
...
|---|---:|---|---|---|---|
| `public/assets/sprites2d/source/**` 제작 원본·방향 sheet | 18 | `generated` | `imagegen`, `imagegen-directional-source-v1`; `tools/asset-manifest.json` | authoring 전용, 제출 allowlist 제외 | 원본 ImageGen 출력·후처리 권리, 썸네일/영상에 무심코 사용될 위험 |
...
| `tools/submission-assets.mjs` | 현재 Pages/runtime에 복사할 73개 allowlist와 authoring·legacy 제외 의도 | 실제 배포가 그 allowlist만 사용하는지에 대한 최종 호스팅 확인, 권리 승인 |
...
- [ ] AS-01~AS-73 각각에 대해 실제 제작자/권리 주체와 제출 권한을 대표자가 확인한다.
...
- [ ] AS-03, AS-05~AS-13, AS-15의 11개 source chain은 source 원본·runtime 파생물·정확한 변환 명령·입력/출력 SHA를 하나의 immutable run/build 기록으로 묶는다. source SHA match를 파생물 권리 승인으로 확장하지 않는다.
...
- [ ] 3D/TRELLIS/Forge 자산을 최종 런타임·썸네일·3분 영상에 넣을 경우, 이 원장의 별도 row를 만들고 ImageGen 원천 및 외부 서비스 권리를 먼저 닫는다. 현재 계획상 제출 runtime은 2D allowlist 73개다.
...
- [ ] build 산출물은 `tools/submission-assets.mjs` allowlist를 사용했는지 확인하고, allowlist 밖 44개가 generic `public/**` 복사·host 설정으로 다시 포함되지 않았는지 실제 배포에서 검증한다.
- [ ] 최종 build/deploy ID, `run_id`, 제출 자산 SHA-256, 권리 증거 경로를 하나의 불변 기록으로 묶는다. 자동 audit/테스트/시각 QA는 권리 승인을 대신하지 않는다.
- [ ] 최종 제출 커밋을 만든 뒤 73개 파일의 SHA-256·크기·mtime과 실제 dist/공개 URL을 다시 캡처한다. 현재 snapshot의 untracked/dirty 상태를 제출 증거로 사용하지 않는다.
...
```text
asset_id: <AS-01..AS-21 또는 THUMBNAIL/DEMO_VIDEO>
...
- OpenAI 약관의 Output 조항은 기술 chain이 연결된 73/73의 **조건부 후보 근거**일 뿐이다. 로컬 생성물에는 계정·관할·서비스·약관 동의 시점이 완전하게 남아 있지 않으므로, 해당 출력이 실제 그 약관의 적용을 받는다는 사실은 아직 확인되지 않았다.
- 8개 direct output, 14개 sprite source chain, 19개 release-v3 UI chain, 32개 v5 의미 아이콘 chain 모두 Input 원천, 제3자 이미지/캐릭터/로고, 실존 인물 유사성, 상표·개인정보 검토 기록이 없다. OpenAI 약관도 Input 권리와 사람 검토 책임을 사용자에게 둔다.
...
- 기존 11개 sprite 파생물은 `build_sprite_atlas.py`/`clean_chroma_spill.py`의 존재와 `sourceUrl` 연결만 확인했다. AS별 정확한 명령·입력·출력 log가 없어, 새로 완전 재현한 AS-04·AS-14·AS-16과 같은 수준의 실행 증거는 아니다.
- 현재 final asset 중 14개 sprite와 AS-02/AS-21/AS-73은 untracked이고 작업 트리가 dirty다. 고정된 제출 commit/build/deploy/run이 없으므로 프로젝트 커밋·팀 소유권·대표자 제출 권한은 미확인이다.
...
```text
submission_runtime_files: 73
technical_classification: generated=73, own=0, external-direct=0, unknown-source=0
local_exact_direct_imagegen_match: 8/73
local_exact_sprite_source_chain: 11/73
local_reproduced_sprite_source_chain: 3/73 (AS-04, AS-14, AS-16)
local_release_v3_ui_atlas_chain: 19/73
local_release_v5_semantic_icon_atlas_chain: 32/73
source_original_not_found: 0/73
technical_provenance_chain_verified: 73/73
conditional_openai_terms_basis: 73/73 (not a rights clearance)
rights_evidence_confirmed: 0/73
rights_gate: BLOCKED

...
- `.codex/generated_images`와 `artifacts/2d-build`의 파일·SHA-256은 로컬 출력 보관·동일성 증거일 뿐, 어느 OpenAI 계정/관할/서비스 약관으로 생성됐는지, Input 권리가 있었는지, 팀 대표자가 제출·홍보권을 갖는지는 증명하지 않는다. 그러므로 위 OpenAI 약관을 근거로 78개 중 어느 것도 `own` 또는 권리 확인으로 승격하지 않는다.
...
| 확인 항목 | 현재 관찰값 | 증거 | 판정 |
|---|---:|---|---|
| 매니페스트 자산 | 127개 | `tools/asset-manifest.json` | 기술 목록 확인 |
| 실제 `public/assets` 파일 | 127개 | `public/assets/**` 재귀 목록 | 매니페스트와 수량 일치 |
| 현재 제출 runtime allowlist | 78개 | `tools/submission-assets.mjs`의 `SUBMISSION_RUNTIME_ASSETS` | 아래 78개만 계획상 번들 포함 |
| allowlist 밖 public 자산 | 49개 | 매니페스트와 allowlist 차집합 | 제출 번들에서 제외하도록 설계됨. generic `public/**` 배포로 재포함되지 않는지 별도 확인 필요 |
| 권리 증거 파일 | 법적 권리 증거 없음; 기술 provenance는 78/78 추적 | 직접 출력 8/78, sprite source chain 19/78(원본과 source exact 16 + 원본·생성 로그·변환 재현 3), release-v3 UI atlas chain 19/78, release-v5 의미별 아이콘 atlas chain 32/78, exact original 미검출 0/78. 계정·Input 권리·대표자 권한·대회 공개 허락 기록은 없음 | 기술 provenance 78/78 확인; 법적 권리 0/78·차단 |
...
아래 78개는 현재 `tools/submission-assets.mjs` allowlist와 매니페스트의 교집합이다. 전부 기술 provenance는 `generated`로 분류한다. 상태 문구의 `로컬 원본 확인` 또는 `source chain 확인`은 파일 동일성과 경로가 확인됐다는 뜻일 뿐, 권리 상태가 `own` 또는 제출 승인으로 바뀌었다는 뜻이 아니다. `외부 파이프라인` 표시는 출력 파일의 외부 사용권이 확인됐다는 뜻이 아니다.
...
| AS-78 | `public/assets/sprites2d/jade-ridge-hound-motion-v1.png` | `generated` | `imagegen-built-in-chroma-key-alpha-despill-atlas-normalize-v1` | 일반 늑대 군중의 짧은 꼬리·비취 등마루 변형 4×2 이동·공격 atlas; `src/runtime2d/spriteManifest.js`, `src/runtime2d/PixiPresentation.js` | local ImageGen source/original SHA와 runtime 파생물 SHA·후처리·실제 1920×1080/2560×1600의 72체 집중 시각 확인은 기록했지만 생성 계정·Input 권리·제3자 유사성·공개·상업·대회 사용과 팀 권한 증거는 없음 | **로컬 source chain 확인·법적 권리 미확인·차단** |
...
#### 비취 등마루 사냥개 runtime fingerprint — AS-78

| ID | 제출 파일 | 바이트 | SHA-256 | mtime UTC | Git 상태/최근 커밋 |
|---|---|---:|---|---|---|
| AS-78 | `public/assets/sprites2d/jade-ridge-hound-motion-v1.png` | 312,077 | `12893c8b51346130fcc24d5cccb65a924c8912b8f6416a3114fec37723c0be01` | `2026-08-11T22:58:29.4959863Z` | 이번 품질 체크포인트에 포함 예정 |

ImageGen 원본 `exec-6ebfc200-d71f-4be3-9b45-d86074273c89.png`와 authoring source의 SHA-256은 모두 `12dd00c50f9662e8673041fd9e7486c0c3cc9ad2ec0274a07e86700bce858cc8`이다. runtime atlas는 명시적 `#00ff00` chroma soft matte·despill, alpha 보존 RGB gamma 0.85 중간톤 보정, 4×2 cell 256·gutter 6·guard 8 정규화 파생물이다. 실제 Chrome 및 정확한 headless Chrome 1920×1080·2560×1600의 72체 집중 화면에서 기존 장꼬리 푸른 늑대와 새 짧은 꼬리·비취 등마루 실루엣의 분배·접지·크로마 잔여를 확인했지만 법적 권리 상태는 계속 차단한다.
...
| AS-78 | `spriteManifest.js` → `source/jade-ridge-hound-motion-sheet-v1.png` | source/original: 1,546,021 bytes; SHA `12dd00c50f9662e8673041fd9e7486c0c3cc9ad2ec0274a07e86700bce858cc8`; original `C:\Users\50106\.codex\generated_images\019fe2bc-1ddb-7390-910b-8a41a70aa5f0\exec-6ebfc200-d71f-4be3-9b45-d86074273c89.png`, same SHA; runtime SHA `12893c8b51346130fcc24d5cccb65a924c8912b8f6416a3114fec37723c0be01` | source SHA와 original exact match; runtime은 authoring manifest에 기록한 명시적 `#00ff00` soft matte·despill·RGB gamma 0.85 중간톤 보정·4×2 atlas 정규화 파생물 |

#### 생성·후처리 스크립트와 증거 한계

| 경로 | 확인된 역할 | 현재 증거/한계 |
|---|---|---|
| `tools/asset-manifest.json` | 127개 자산의 `path`, `source`, `consumer`, 용량 제한을 선언 | 기술 목록만 증명하며 생성 계정·Input 권리·출력 약관·정확한 명령 실행은 기록하지 않음; 현재 작업 트리에서 수정 상태 |
...
현재 저장소에는 78개 결과 각각의 계정·Input 권리·대표자 공개 권한을 고정한 법적 증거가 없다. 그러므로 `sourceUrl`, atlas crop, exact SHA match를 “로컬 provenance 부분 확인”까지만 사용하고 “권리 승인”으로 확장하지 않는다.
...
- AS-74~AS-78은 이번 품질 체크포인트의 runtime/source fingerprint와 변환 명령을 위 원장에 추가했다. 고정 제출 commit/build/deploy/run과 대표자 권리 증거는 별도 최종 게이트에서 다시 연결해야 한다.
...
| 분류 | 개수 | 의미 |
|---|---:|---|
| `generated` | 78 | 매니페스트가 ImageGen 또는 ImageGen 후처리 provenance를 명시 |
| `own` | 0 | 저장소에서 권리 주체를 확인하는 명시적 소유·양도 증거를 찾지 못함 |
| `external` | 0개의 직접 제출 파일 | 직접 외부 원본 파일은 allowlist에 없지만, 생성 서비스·도구 의존성은 아래 별도 게이트 대상 |
| `unknown` | 0개의 기술 원천 | 기술 source 라벨은 있으나 **권리 상태는 78개 전부 미확인** |
| 직접 제출 파일과 `.codex/generated_images` exact match | 8/78 | AS-01, AS-02, AS-17~AS-21, AS-73의 제출 파일 자체가 local generated original과 SHA-256 일치 |
| sprite source와 `.codex/generated_images` exact match | 16/78 | AS-03, AS-05~AS-13, AS-15, AS-74~AS-78의 source sheet가 local generated original과 SHA-256 일치; runtime은 파생물 |
| 원본·생성 이벤트·변환 명령으로 재현한 sprite chain | 3/78 | AS-04, AS-14, AS-16의 source와 runtime을 격리 재생성하여 파일 SHA와 RGBA pixel 모두 exact match |
| release-v3 UI atlas source chain | 19/78 | AS-22~AS-40은 로컬 ImageGen atlas와 결정적 crop 산출물로 연결됨 |
| release-v5 의미별 아이콘 atlas source chain | 32/78 | AS-41~AS-72는 두 로컬 ImageGen atlas와 결정적 256×256 crop 산출물로 연결됨 |
| source는 있으나 exact original 미검출 | 0/78 | 기존 AS-04, AS-14, AS-16 원본과 변환 chain을 복구·재현함 |
| 기술 provenance chain 확인 | 78/78 | 8 direct + 16 exact sprite source + 3 reproduced sprite chain + 19 legacy UI atlas + 32 semantic icon atlas |
| OpenAI 약관의 조건부 Output 근거 후보 | 78/78 | 로컬 생성 chain이 연결된 범위일 뿐 계정·Input·대회 권리 clearance 아님 |
| 법적 권리 증거 확인 | 0/78 | OpenAI 약관의 일반적 Output 조항, 로컬 SHA, QA PASS는 자산별 권리·대회 제출 허락을 증명하지 않음 |

이 표의 `own=0`, `unknown=0`은 기술 source 라벨의 분류이며 안전 판정이 아니다. provenance 연결 수는 권리 승인 수가 아니다. 계정·Input·인물/상표·팀/대표자 권한·대회 공개 허락이 확인되지 않은 상태이므로 제출 가능 수와 최종 권리 확인 수는 현재 0/78로 취급한다.

## allowlist 밖 49개와 외부 파이프라인 위험
...
|---|---:|---|---|---|---|
| `public/assets/sprites2d/source/**` 제작 원본·방향 sheet | 23 | `generated` | `imagegen`, `imagegen-directional-source-v1`; `tools/asset-manifest.json` | authoring 전용, 제출 allowlist 제외 | 원본 ImageGen 출력·후처리 권리, 썸네일/영상에 무심코 사용될 위험 |
...
| `tools/submission-assets.mjs` | 현재 Pages/runtime에 복사할 78개 allowlist와 authoring·legacy 제외 의도 | 실제 배포가 그 allowlist만 사용하는지에 대한 최종 호스팅 확인, 권리 승인 |
...
- [ ] AS-01~AS-78 각각에 대해 실제 제작자/권리 주체와 제출 권한을 대표자가 확인한다.
...
- [ ] AS-03, AS-05~AS-13, AS-15, AS-74~AS-78의 16개 source chain은 source 원본·runtime 파생물·정확한 변환 명령·입력/출력 SHA를 하나의 immutable run/build 기록으로 묶는다. source SHA match를 파생물 권리 승인으로 확장하지 않는다.
...
- [ ] 3D/TRELLIS/Forge 자산을 최종 런타임·썸네일·3분 영상에 넣을 경우, 이 원장의 별도 row를 만들고 ImageGen 원천 및 외부 서비스 권리를 먼저 닫는다. 현재 계획상 제출 runtime은 2D allowlist 78개다.
...
- [ ] build 산출물은 `tools/submission-assets.mjs` allowlist를 사용했는지 확인하고, allowlist 밖 49개가 generic `public/**` 복사·host 설정으로 다시 포함되지 않았는지 실제 배포에서 검증한다.
- [ ] 최종 build/deploy ID, `run_id`, 제출 자산 SHA-256, 권리 증거 경로를 하나의 불변 기록으로 묶는다. 자동 audit/테스트/시각 QA는 권리 승인을 대신하지 않는다.
- [ ] 최종 제출 커밋을 만든 뒤 78개 파일의 SHA-256·크기·mtime과 실제 dist/공개 URL을 다시 캡처한다. 현재 snapshot의 untracked/dirty 상태를 제출 증거로 사용하지 않는다.
...
```text
asset_id: <AS-01..AS-78 또는 THUMBNAIL/DEMO_VIDEO>
...
- OpenAI 약관의 Output 조항은 기술 chain이 연결된 78/78의 **조건부 후보 근거**일 뿐이다. 로컬 생성물에는 계정·관할·서비스·약관 동의 시점이 완전하게 남아 있지 않으므로, 해당 출력이 실제 그 약관의 적용을 받는다는 사실은 아직 확인되지 않았다.
- 8개 direct output, 19개 sprite source chain, 19개 release-v3 UI chain, 32개 v5 의미 아이콘 chain 모두 Input 원천, 제3자 이미지/캐릭터/로고, 실존 인물 유사성, 상표·개인정보 검토 기록이 없다. OpenAI 약관도 Input 권리와 사람 검토 책임을 사용자에게 둔다.
...
- 기존 11개 sprite 파생물은 `build_sprite_atlas.py`/`clean_chroma_spill.py`의 존재와 `sourceUrl` 연결만 확인했다. AS-74~AS-78은 별도 chroma-key·soft-matte 변환 명령을 기록했지만 immutable build/run 묶음은 아직 없다. 따라서 AS-04·AS-14·AS-16의 완전 재현 증거와 같은 법적 승격 근거로 사용하지 않는다.
- release-v3 snapshot에서 sprite와 AS-02/AS-21/AS-73은 제출 기준 commit/build/deploy/run이 고정되지 않았다. 현재 AS-78까지 포함한 새 체크포인트도 최종 제출 commit·공개 build·동일 run ID와 대표자 제출 권한을 별도로 고정하기 전에는 권리 승인으로 취급하지 않는다.
...
```text
submission_runtime_files: 78
technical_classification: generated=78, own=0, external-direct=0, unknown-source=0
local_exact_direct_imagegen_match: 8/78
local_exact_sprite_source_chain: 16/78
local_reproduced_sprite_source_chain: 3/78 (AS-04, AS-14, AS-16)
local_release_v3_ui_atlas_chain: 19/78
local_release_v5_semantic_icon_atlas_chain: 32/78
source_original_not_found: 0/78
technical_provenance_chain_verified: 78/78
conditional_openai_terms_basis: 78/78 (not a rights clearance)
rights_evidence_confirmed: 0/78
rights_gate: BLOCKED

...
- `src/art/SeolryeongImg2ThreeAdapter.js`가 위 팩토리와 GitHub 공식 파이프라인의 결과를 플레이어 런타임 그래프에 연결합니다. ImageGen v4 4방향 turnaround를 공식 `integrations/mesh3d/generate_reference_mesh.py`의 TRELLIS 멀티뷰 입력으로 넣어 `seolryeong-trellis-v4.glb`를 만들었고, 현재 전투에서는 이 66,087-triangle GLB가 보이는 영웅 모델입니다. 기존 `HeroicModels.js` 셸은 GLB 로딩 실패 시에만 fallback으로 표시됩니다.
...
- strict img2threejs sculpt gate는 아직 통과하지 않았습니다. 공식 factory는 런타임 그래프에 구조/소켓 기준으로 숨겨 연결하고, 실제 보이는 영웅은 ImageGen 멀티뷰를 TRELLIS로 변환한 GLB를 사용합니다. 현재 적 모델은 `near-detail refinement`이며 AAA/고품질 skinned GLB로 표시하지 않습니다.
...
- 지연 로딩 적용 후 새로고침 → 비경 진입 → 청람비경 → 설령 → 전투를 다시 실행했습니다. F3 계측에서 `60 FPS / 16.7 ms`, `work 3.0 ms`, `sim/draw 0.2/2.8 ms`, `54 enemies`, `742,449 tris`, `dropped 0`, `WebGL2`, `warmup 197.9 ms`를 확인했고 브라우저 오류/경고는 0건이었습니다. 전투 중앙에는 v4 TRELLIS GLB 설령이 실제로 표시됩니다. 이는 게임 내부 렌더 계측이며 Windows 작업 관리자 전체 CPU 사용률을 대신하지 않습니다.
...
- 캐릭터의 기존 ObjectSculpt factory는 여전히 `blockout` 구조 기준으로 기록되어 있습니다. 이번에 추가한 TRELLIS GLB는 실제 브라우저에서 보이는 멀티뷰 생성형 proxy이지만, 스켈레톤/애니메이션/수동 토폴로지 검수까지 끝난 AAA 최종 에셋은 아닙니다. 따라서 적용은 완료했지만 AAA 완료로 표시하지 않습니다.

...
const loadedMapTextureCache = new Map<string, THREE.Texture>();

function resolveRuntimeMapUrl(url: string): string {
...
function createLoadedMapTexture(
  ...
  const runtimeUrl = resolveRuntimeMapUrl(url);
  const cacheKey = `${runtimeUrl}|${colorSpace}`;
  const cached = loadedMapTextureCache.get(cacheKey);
  if (cached) return cached;
  const texture = new THREE.TextureLoader().load(runtimeUrl);
  const projection = spec.textureProjection && typeof spec.textureProjection === 'object' ? spec.textureProjection : {};
  ...
  loadedMapTextureCache.set(cacheKey, texture);
  return texture;
}

function makeReferenceTextureSet(id: string, spec: SculptMaterialSpec, options: ProceduralModelOptions): ProceduralTextureSet | null {
  // The extracted maps are moon-silk evidence. Keep skin, hair, eyes, lips,
  // and boots on their own procedural material maps so the character does not
  // collapse into one beige/blue texture.
  if (!new Set(['base', 'shirt', 'pants']).has(id)) return null;
...
function createSculptMaterial(id: string, spec: SculptMaterialSpec, options: ProceduralModelOptions, denseComponent = false): THREE.MeshPhysicalMaterial {
  // Only the robe materials need the extracted silk maps in the live horde.
  // Generating five 256+ procedural canvases for every skin/hair/eye part at
  // startup created a multi-million-pixel CPU spike before the first frame.
  // Those parts keep their authored physical color; silk remains reference
  // driven and cached.
  const usesSilkRuntimeMaps = new Set(['base', 'shirt', 'pants']).has(id);
  const textures = usesSilkRuntimeMaps
    ? (makeReferenceTextureSet(id, spec, options) ?? makeProceduralTextureSet(id, spec, options))
    : null;
...
// Generated from ObjectSculptSpec target: Seolryeong frost-silk swordswoman
// Sculpt build pass: blockout
// This factory is intentionally pass-gated. Finish browser screenshot review before unlocking deeper passes.
export function createSeolryeongFrostSilkSwordswomanModel(options: ProceduralModelOptions = {}): THREE.Group {
  const root = new THREE.Group();
  root.name = "Seolryeong frost-silk swordswoman";
  root.userData.reconstructionEvidence = {"itemFamily": null, "subtype": null, "componentAdapter": null, "route": null, "exactnessTier": null, "referenceCamera": {"solved": true, "projection": "perspective", "azimuthDeg": 14.0, "elevationDeg": 2.0, "fovDeg": 36.0, "subjectCenter": [0.0, 0.54, 0.0], "framingMargin": 1.12, "confidence": 0.84, "evidenceRef": "full-object"}, "approximationNotes": []};
  ...
  materialMap["base"] = createSculptMaterial(
    "base",
    {"id": "base", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#DCEEFF", "color": "#DCEEFF", "albedo": {"dominant": "#DCEEFF", "secondary": ["#91B6D8"]}, "colorVariation": {"palette": ["#DCEEFF", "#91B6D8"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.56, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.18, "scratches": ["micro-scratches on silver trim and sword hardware"], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "base-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#91B6D8", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "reference-silk", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["hidden"] = createSculptMaterial(
    "hidden",
    {"id": "hidden", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#0B101A", "color": "#0B101A", "albedo": {"dominant": "#0B101A", "secondary": ["#182338"]}, "colorVariation": {"palette": ["#0B101A", "#182338"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.38, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.18, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "hidden-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#182338", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "opacity": {"base": 0.0}, "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "utility", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["skin"] = createSculptMaterial(
    "skin",
    {"id": "skin", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#F1C2B2", "color": "#F1C2B2", "albedo": {"dominant": "#F1C2B2", "secondary": ["#D88E8C"]}, "colorVariation": {"palette": ["#F1C2B2", "#D88E8C"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.38, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.02, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "skin-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#D88E8C", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["hair"] = createSculptMaterial(
    "hair",
    {"id": "hair", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#17243A", "color": "#17243A", "albedo": {"dominant": "#17243A", "secondary": ["#5C7195"]}, "colorVariation": {"palette": ["#17243A", "#5C7195"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.56, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.18, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "hair-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#5C7195", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["shirt"] = createSculptMaterial(
    "shirt",
    {"id": "shirt", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#F1F6FF", "color": "#F1F6FF", "albedo": {"dominant": "#F1F6FF", "secondary": ["#86A9D0"]}, "colorVariation": {"palette": ["#F1F6FF", "#86A9D0"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.56, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.18, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "shirt-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#86A9D0", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}, {"id": "shirt-silk-weave", "region": "robe shell and sleeve fabric", "normalStrength": 0.22, "roughnessDelta": 0.12, "strength": 0.78, "evidenceRef": "costume-close"}, {"id": "shirt-embroidered-trim", "region": "collar, hem, and sleeve border", "albedoShift": "#86A9D0", "roughnessDelta": -0.06, "strength": 0.72, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "reference-silk", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["pants"] = createSculptMaterial(
    "pants",
    {"id": "pants", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#DCE8F5", "color": "#DCE8F5", "albedo": {"dominant": "#DCE8F5", "secondary": ["#607EA5"]}, "colorVariation": {"palette": ["#DCE8F5", "#607EA5"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.56, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.18, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "pants-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#607EA5", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "reference-silk", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["shoes"] = createSculptMaterial(
    "shoes",
    {"id": "shoes", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#DDE9F7", "color": "#DDE9F7", "albedo": {"dominant": "#DDE9F7", "secondary": ["#6F8FAE"]}, "colorVariation": {"palette": ["#DDE9F7", "#6F8FAE"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.38, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.18, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "shoes-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#6F8FAE", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}, {"id": "shoes-boot-engraving", "region": "front boot panel", "albedoShift": "#8BB0D2", "normalStrength": 0.16, "strength": 0.62, "evidenceRef": "lower-silhouette"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["eye"] = createSculptMaterial(
    "eye",
    {"id": "eye", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#87E8FF", "color": "#87E8FF", "albedo": {"dominant": "#87E8FF", "secondary": ["#244D7A"]}, "colorVariation": {"palette": ["#87E8FF", "#244D7A"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.38, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.02, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "eye-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#244D7A", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  materialMap["lips"] = createSculptMaterial(
    "lips",
    {"id": "lips", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#D8798A", "color": "#D8798A", "albedo": {"dominant": "#D8798A", "secondary": ["#9E465D"]}, "colorVariation": {"palette": ["#D8798A", "#9E465D"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.38, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.02, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "lips-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#9E465D", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}},
    options
  );
  ...
  const mesh_pelvis_1Geometry = endpoint_pelvis_1
    ? new THREE.CylinderGeometry(endpoint_pelvis_1.endRadius, endpoint_pelvis_1.baseRadius, endpoint_pelvis_1.length, 16, 6)
  ...
  const mesh_abdomen_2Geometry = endpoint_abdomen_2
    ? new THREE.CylinderGeometry(endpoint_abdomen_2.endRadius, endpoint_abdomen_2.baseRadius, endpoint_abdomen_2.length, 16, 6)
  ...
  const mesh_neck_4Geometry = endpoint_neck_4
    ? new THREE.CylinderGeometry(endpoint_neck_4.endRadius, endpoint_neck_4.baseRadius, endpoint_neck_4.length, 16, 6)
  ...
  const mesh_hair_6Geometry = endpoint_hair_6
    ? new THREE.CylinderGeometry(endpoint_hair_6.endRadius, endpoint_hair_6.baseRadius, endpoint_hair_6.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_hair_6) {
  ...
  const mesh_brow_l_7Geometry = endpoint_brow_l_7
    ? new THREE.CylinderGeometry(endpoint_brow_l_7.endRadius, endpoint_brow_l_7.baseRadius, endpoint_brow_l_7.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_brow_l_7) {
  ...
  const mesh_brow_r_8Geometry = endpoint_brow_r_8
    ? new THREE.CylinderGeometry(endpoint_brow_r_8.endRadius, endpoint_brow_r_8.baseRadius, endpoint_brow_r_8.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_brow_r_8) {
  ...
  const mesh_ear_l_9Geometry = endpoint_ear_l_9
    ? new THREE.CylinderGeometry(endpoint_ear_l_9.endRadius, endpoint_ear_l_9.baseRadius, endpoint_ear_l_9.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_ear_l_9) {
  ...
  const mesh_ear_r_10Geometry = endpoint_ear_r_10
    ? new THREE.CylinderGeometry(endpoint_ear_r_10.endRadius, endpoint_ear_r_10.baseRadius, endpoint_ear_r_10.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_ear_r_10) {
  ...
  const mesh_nose_11Geometry = endpoint_nose_11
    ? new THREE.CylinderGeometry(endpoint_nose_11.endRadius, endpoint_nose_11.baseRadius, endpoint_nose_11.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_nose_11) {
  ...
  const mesh_mouth_12Geometry = endpoint_mouth_12
    ? new THREE.CylinderGeometry(endpoint_mouth_12.endRadius, endpoint_mouth_12.baseRadius, endpoint_mouth_12.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_mouth_12) {
  ...
  const mesh_eye_l_13Geometry = endpoint_eye_l_13
    ? new THREE.CylinderGeometry(endpoint_eye_l_13.endRadius, endpoint_eye_l_13.baseRadius, endpoint_eye_l_13.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_eye_l_13) {
  ...
  const mesh_eye_cavity_l_14 = new THREE.Mesh(
    ...
    createSculptMaterial("skin", {"id": "skin", "name": "Base material", "type": "standard", "shaderModel": "MeshStandardMaterial / PBR approximation", "baseColor": "#F1C2B2", "color": "#F1C2B2", "albedo": {"dominant": "#F1C2B2", "secondary": ["#D88E8C"]}, "colorVariation": {"palette": ["#F1C2B2", "#D88E8C"], "pattern": "reference-derived macro/meso breakup", "amplitude": 0.12, "heightCorrelation": 0.35}, "textureResolution": 1024, "textureProjection": {"mode": "uv", "repeat": [2.0, 2.0], "anisotropy": 8, "texelDensityIntent": "stable object-space detail; never stretch the motif with the component pivot"}, "surfaceFrequencyBands": [{"id": "macro", "frequency": 2.0, "amplitude": 0.42, "role": "broad color and height breakup"}, {"id": "meso", "frequency": 12.0, "amplitude": 0.22, "role": "ridges, pores, grain, dents, or equivalent visible relief"}, {"id": "micro", "frequency": 56.0, "amplitude": 0.08, "role": "highlight breakup visible under grazing light"}], "roughness": {"base": 0.38, "variation": 0.16, "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}}, "metalness": {"base": 0.08, "variation": 0.08}, "normal": {"pattern": "independent reference-derived relief", "strength": 0.22, "scale": 24.0, "space": "tangent", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}}, "bump": {"pattern": "weave-and-seam micro relief", "amplitude": 0.08, "scale": 18.0}, "displacement": {"pattern": "none", "amplitude": 0.0, "scale": 1.0, "silhouetteAffects": false}, "ambientOcclusion": {"cavityStrength": 0.34, "contactShadowBias": 0.25, "notes": "darken robe overlaps, hair roots, sword guard, and boot seams", "map": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}, "wear": {"edgeWear": 0.02, "scratches": [], "chips": []}, "dirt": {"amount": 0.05, "cavityBias": 0.18, "color": "#293546"}, "localOverrides": [{"id": "skin-seam-contrast", "region": "visible costume seam and contact zones", "albedoShift": "#D88E8C", "roughnessDelta": 0.08, "strength": 0.62, "evidenceRef": "costume-close"}], "shaderNotes": ["Use MeshPhysicalMaterial clearcoat only for silver trim and wet ice; keep cloth sheen restrained.", "Keep albedo, roughness, normal, height, and AO independent.", "Tone-map the pale cloth before adding emissive frost accents."], "notes": "Replace with image-derived color, roughness, noise, and edge-wear notes.", "runtimeTexturePolicy": "procedural-material-specific", "qualityTier": "hero", "referencePbr": {"version": "1.0", "sourceImage": "artifacts\\img2threejs\\seolryeong\\character-model\\seolryeong-isolated-reference.png", "extractor": "img2threejs forge/stage1_intake/extract_pbr_evidence.py", "method": "reference-pixel extraction; estimated PBR evidence, not inverse rendering", "verdict": "pass-with-single-view-limitations", "hardLimit": "single image cannot prove hidden-side geometry or ground-truth material channels", "usable": true, "confidence": 0.86, "estimatedFidelity": 0.86, "targetThreshold": 0.7, "maps": {"albedo": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_albedo.png", "url": "seolryeong-silk_albedo.png", "channel": "albedo", "source": "img2threejs forge reference-pixel extraction"}, "roughness": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_roughness.png", "url": "seolryeong-silk_roughness.png", "channel": "roughness", "source": "img2threejs forge reference-pixel extraction"}, "height": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_height.png", "url": "seolryeong-silk_height.png", "channel": "height", "source": "img2threejs forge reference-pixel extraction"}, "normal": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_normal.png", "url": "seolryeong-silk_normal.png", "channel": "normal", "source": "img2threejs forge reference-pixel extraction"}, "ao": {"path": "artifacts\\img2threejs\\seolryeong\\character-model\\pbr-evidence-isolated\\seolryeong-silk_ao.png", "url": "seolryeong-silk_ao.png", "channel": "ao", "source": "img2threejs forge reference-pixel extraction"}}}}, options, true)
  );
  ...
  const mesh_eye_r_15Geometry = endpoint_eye_r_15
    ? new THREE.CylinderGeometry(endpoint_eye_r_15.endRadius, endpoint_eye_r_15.baseRadius, endpoint_eye_r_15.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_eye_r_15) {
  ...
  const mesh_clavicle_l_17Geometry = endpoint_clavicle_l_17
    ? new THREE.CylinderGeometry(endpoint_clavicle_l_17.endRadius, endpoint_clavicle_l_17.baseRadius, endpoint_clavicle_l_17.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_clavicle_l_17) {
  ...
  const mesh_upper_arm_l_18Geometry = endpoint_upper_arm_l_18
    ? new THREE.CylinderGeometry(endpoint_upper_arm_l_18.endRadius, endpoint_upper_arm_l_18.baseRadius, endpoint_upper_arm_l_18.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_upper_arm_l_18) {
  ...
  const mesh_forearm_l_19Geometry = endpoint_forearm_l_19
    ? new THREE.CylinderGeometry(endpoint_forearm_l_19.endRadius, endpoint_forearm_l_19.baseRadius, endpoint_forearm_l_19.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_forearm_l_19) {
  ...
  const mesh_hand_l_20Geometry = endpoint_hand_l_20
    ? new THREE.CylinderGeometry(endpoint_hand_l_20.endRadius, endpoint_hand_l_20.baseRadius, endpoint_hand_l_20.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_hand_l_20) {
  ...
  const mesh_thumb_l_1_21Geometry = endpoint_thumb_l_1_21
    ? new THREE.CylinderGeometry(endpoint_thumb_l_1_21.endRadius, endpoint_thumb_l_1_21.baseRadius, endpoint_thumb_l_1_21.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thumb_l_1_21) {
  ...
  const mesh_thumb_l_2_22Geometry = endpoint_thumb_l_2_22
    ? new THREE.CylinderGeometry(endpoint_thumb_l_2_22.endRadius, endpoint_thumb_l_2_22.baseRadius, endpoint_thumb_l_2_22.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thumb_l_2_22) {
  ...
  const mesh_thumb_l_3_23Geometry = endpoint_thumb_l_3_23
    ? new THREE.CylinderGeometry(endpoint_thumb_l_3_23.endRadius, endpoint_thumb_l_3_23.baseRadius, endpoint_thumb_l_3_23.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thumb_l_3_23) {
  ...
  const mesh_index_l_1_24Geometry = endpoint_index_l_1_24
    ? new THREE.CylinderGeometry(endpoint_index_l_1_24.endRadius, endpoint_index_l_1_24.baseRadius, endpoint_index_l_1_24.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_index_l_1_24) {
  ...
  const mesh_index_l_2_25Geometry = endpoint_index_l_2_25
    ? new THREE.CylinderGeometry(endpoint_index_l_2_25.endRadius, endpoint_index_l_2_25.baseRadius, endpoint_index_l_2_25.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_index_l_2_25) {
  ...
  const mesh_index_l_3_26Geometry = endpoint_index_l_3_26
    ? new THREE.CylinderGeometry(endpoint_index_l_3_26.endRadius, endpoint_index_l_3_26.baseRadius, endpoint_index_l_3_26.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_index_l_3_26) {
  ...
  const mesh_middle_l_1_27Geometry = endpoint_middle_l_1_27
    ? new THREE.CylinderGeometry(endpoint_middle_l_1_27.endRadius, endpoint_middle_l_1_27.baseRadius, endpoint_middle_l_1_27.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_middle_l_1_27) {
  ...
  const mesh_middle_l_2_28Geometry = endpoint_middle_l_2_28
    ? new THREE.CylinderGeometry(endpoint_middle_l_2_28.endRadius, endpoint_middle_l_2_28.baseRadius, endpoint_middle_l_2_28.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_middle_l_2_28) {
  ...
  const mesh_middle_l_3_29Geometry = endpoint_middle_l_3_29
    ? new THREE.CylinderGeometry(endpoint_middle_l_3_29.endRadius, endpoint_middle_l_3_29.baseRadius, endpoint_middle_l_3_29.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_middle_l_3_29) {
  ...
  const mesh_ring_l_1_30Geometry = endpoint_ring_l_1_30
    ? new THREE.CylinderGeometry(endpoint_ring_l_1_30.endRadius, endpoint_ring_l_1_30.baseRadius, endpoint_ring_l_1_30.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_ring_l_1_30) {
  ...
  const mesh_ring_l_2_31Geometry = endpoint_ring_l_2_31
    ? new THREE.CylinderGeometry(endpoint_ring_l_2_31.endRadius, endpoint_ring_l_2_31.baseRadius, endpoint_ring_l_2_31.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_ring_l_2_31) {
  ...
  const mesh_ring_l_3_32Geometry = endpoint_ring_l_3_32
    ? new THREE.CylinderGeometry(endpoint_ring_l_3_32.endRadius, endpoint_ring_l_3_32.baseRadius, endpoint_ring_l_3_32.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_ring_l_3_32) {
  ...
  const mesh_little_l_1_33Geometry = endpoint_little_l_1_33
    ? new THREE.CylinderGeometry(endpoint_little_l_1_33.endRadius, endpoint_little_l_1_33.baseRadius, endpoint_little_l_1_33.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_little_l_1_33) {
  ...
  const mesh_little_l_2_34Geometry = endpoint_little_l_2_34
    ? new THREE.CylinderGeometry(endpoint_little_l_2_34.endRadius, endpoint_little_l_2_34.baseRadius, endpoint_little_l_2_34.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_little_l_2_34) {
  ...
  const mesh_little_l_3_35Geometry = endpoint_little_l_3_35
    ? new THREE.CylinderGeometry(endpoint_little_l_3_35.endRadius, endpoint_little_l_3_35.baseRadius, endpoint_little_l_3_35.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_little_l_3_35) {
  ...
  const mesh_clavicle_r_36Geometry = endpoint_clavicle_r_36
    ? new THREE.CylinderGeometry(endpoint_clavicle_r_36.endRadius, endpoint_clavicle_r_36.baseRadius, endpoint_clavicle_r_36.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_clavicle_r_36) {
  ...
  const mesh_upper_arm_r_37Geometry = endpoint_upper_arm_r_37
    ? new THREE.CylinderGeometry(endpoint_upper_arm_r_37.endRadius, endpoint_upper_arm_r_37.baseRadius, endpoint_upper_arm_r_37.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_upper_arm_r_37) {
  ...
  const mesh_forearm_r_38Geometry = endpoint_forearm_r_38
    ? new THREE.CylinderGeometry(endpoint_forearm_r_38.endRadius, endpoint_forearm_r_38.baseRadius, endpoint_forearm_r_38.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_forearm_r_38) {
  ...
  const mesh_hand_r_39Geometry = endpoint_hand_r_39
    ? new THREE.CylinderGeometry(endpoint_hand_r_39.endRadius, endpoint_hand_r_39.baseRadius, endpoint_hand_r_39.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_hand_r_39) {
  ...
  const mesh_thumb_r_1_40Geometry = endpoint_thumb_r_1_40
    ? new THREE.CylinderGeometry(endpoint_thumb_r_1_40.endRadius, endpoint_thumb_r_1_40.baseRadius, endpoint_thumb_r_1_40.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thumb_r_1_40) {
  ...
  const mesh_thumb_r_2_41Geometry = endpoint_thumb_r_2_41
    ? new THREE.CylinderGeometry(endpoint_thumb_r_2_41.endRadius, endpoint_thumb_r_2_41.baseRadius, endpoint_thumb_r_2_41.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thumb_r_2_41) {
  ...
  const mesh_thumb_r_3_42Geometry = endpoint_thumb_r_3_42
    ? new THREE.CylinderGeometry(endpoint_thumb_r_3_42.endRadius, endpoint_thumb_r_3_42.baseRadius, endpoint_thumb_r_3_42.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thumb_r_3_42) {
  ...
  const mesh_index_r_1_43Geometry = endpoint_index_r_1_43
    ? new THREE.CylinderGeometry(endpoint_index_r_1_43.endRadius, endpoint_index_r_1_43.baseRadius, endpoint_index_r_1_43.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_index_r_1_43) {
  ...
  const mesh_index_r_2_44Geometry = endpoint_index_r_2_44
    ? new THREE.CylinderGeometry(endpoint_index_r_2_44.endRadius, endpoint_index_r_2_44.baseRadius, endpoint_index_r_2_44.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_index_r_2_44) {
  ...
  const mesh_index_r_3_45Geometry = endpoint_index_r_3_45
    ? new THREE.CylinderGeometry(endpoint_index_r_3_45.endRadius, endpoint_index_r_3_45.baseRadius, endpoint_index_r_3_45.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_index_r_3_45) {
  ...
  const mesh_middle_r_1_46Geometry = endpoint_middle_r_1_46
    ? new THREE.CylinderGeometry(endpoint_middle_r_1_46.endRadius, endpoint_middle_r_1_46.baseRadius, endpoint_middle_r_1_46.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_middle_r_1_46) {
  ...
  const mesh_middle_r_2_47Geometry = endpoint_middle_r_2_47
    ? new THREE.CylinderGeometry(endpoint_middle_r_2_47.endRadius, endpoint_middle_r_2_47.baseRadius, endpoint_middle_r_2_47.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_middle_r_2_47) {
  ...
  const mesh_middle_r_3_48Geometry = endpoint_middle_r_3_48
    ? new THREE.CylinderGeometry(endpoint_middle_r_3_48.endRadius, endpoint_middle_r_3_48.baseRadius, endpoint_middle_r_3_48.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_middle_r_3_48) {
  ...
  const mesh_ring_r_1_49Geometry = endpoint_ring_r_1_49
    ? new THREE.CylinderGeometry(endpoint_ring_r_1_49.endRadius, endpoint_ring_r_1_49.baseRadius, endpoint_ring_r_1_49.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_ring_r_1_49) {
  ...
  const mesh_ring_r_2_50Geometry = endpoint_ring_r_2_50
    ? new THREE.CylinderGeometry(endpoint_ring_r_2_50.endRadius, endpoint_ring_r_2_50.baseRadius, endpoint_ring_r_2_50.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_ring_r_2_50) {
  ...
  const mesh_ring_r_3_51Geometry = endpoint_ring_r_3_51
    ? new THREE.CylinderGeometry(endpoint_ring_r_3_51.endRadius, endpoint_ring_r_3_51.baseRadius, endpoint_ring_r_3_51.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_ring_r_3_51) {
  ...
  const mesh_little_r_1_52Geometry = endpoint_little_r_1_52
    ? new THREE.CylinderGeometry(endpoint_little_r_1_52.endRadius, endpoint_little_r_1_52.baseRadius, endpoint_little_r_1_52.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_little_r_1_52) {
  ...
  const mesh_little_r_2_53Geometry = endpoint_little_r_2_53
    ? new THREE.CylinderGeometry(endpoint_little_r_2_53.endRadius, endpoint_little_r_2_53.baseRadius, endpoint_little_r_2_53.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_little_r_2_53) {
  ...
  const mesh_little_r_3_54Geometry = endpoint_little_r_3_54
    ? new THREE.CylinderGeometry(endpoint_little_r_3_54.endRadius, endpoint_little_r_3_54.baseRadius, endpoint_little_r_3_54.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_little_r_3_54) {
  ...
  const mesh_thigh_l_55Geometry = endpoint_thigh_l_55
    ? new THREE.CylinderGeometry(endpoint_thigh_l_55.endRadius, endpoint_thigh_l_55.baseRadius, endpoint_thigh_l_55.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_thigh_l_55) {
  ...
  const mesh_foot_l_57Geometry = endpoint_foot_l_57
    ? new THREE.CylinderGeometry(endpoint_foot_l_57.endRadius, endpoint_foot_l_57.baseRadius, endpoint_foot_l_57.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_foot_l_57) {
  ...
  const mesh_shin_r_59Geometry = endpoint_shin_r_59
    ? new THREE.CylinderGeometry(endpoint_shin_r_59.endRadius, endpoint_shin_r_59.baseRadius, endpoint_shin_r_59.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_shin_r_59) {
  ...
  const mesh_foot_r_60Geometry = endpoint_foot_r_60
    ? new THREE.CylinderGeometry(endpoint_foot_r_60.endRadius, endpoint_foot_r_60.baseRadius, endpoint_foot_r_60.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_foot_r_60) {
  ...
  const attachment_robe_shell_61 = null;
  const endpoint_robe_shell_61 = makeAttachmentEndpoint(attachment_robe_shell_61);
  const node_robe_shell_61 = new THREE.Group();
  node_robe_shell_61.name = "Layered robe shell__pivot";
  node_robe_shell_61.scale.set(1, 1, 1);
  if (endpoint_robe_shell_61) {
    node_robe_shell_61.position.copy(endpoint_robe_shell_61.start);
    node_robe_shell_61.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_robe_shell_61.position.set(0.0, 0.0, 0.0);
    node_robe_shell_61.rotation.set(0.0, 0.0, 0.0);
  }
  node_robe_shell_61.userData.sculptComponent = {"id": "robe-shell", "name": "Layered robe shell", "level": "macro", "role": "robe-panel", "importance": 0.86, "confidence": 0.83, "primitive": "lathe", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "root", "attachment": null, "dimensions": {"width": 0.66, "height": 1.68, "depth": 0.56, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0, 0.0], "scale": [0.66, 1.68, 0.56]}, "actionProfile": {"animationRole": "robe-panel", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "robe-shell-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for robe-panel"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "robe-panel", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shirt", "materialLayers": ["shirt"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["deep center fold", "separated skirt silhouette", "cloth shell catches a broad key light"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["deep center fold", "separated skirt silhouette", "cloth shell catches a broad key light"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "fabric", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_robe_shell_61.userData.actionProfile = {"animationRole": "robe-panel", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "robe-shell-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for robe-panel"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "robe-panel", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["root"] ?? root).add(node_robe_shell_61);
  nodes["robe-shell"] = node_robe_shell_61;
  const mesh_robe_shell_61Geometry = endpoint_robe_shell_61
    ? new THREE.CylinderGeometry(endpoint_robe_shell_61.endRadius, endpoint_robe_shell_61.baseRadius, endpoint_robe_shell_61.length, 16, 6)
    : buildLatheGeometry({"points": [[0.3, -0.5], [0.15, 0.0], [0.3, 0.5]], "segments": 24});
  if (!endpoint_robe_shell_61) {
    mesh_robe_shell_61Geometry.scale(0.66, 1.68, 0.56);
  }
  const mesh_robe_shell_61 = new THREE.Mesh(
    mesh_robe_shell_61Geometry,
    materialMap["shirt"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_robe_shell_61.name = "Layered robe shell";
  if (endpoint_robe_shell_61) {
    mesh_robe_shell_61.position.copy(endpoint_robe_shell_61.midpoint);
    mesh_robe_shell_61.quaternion.copy(endpoint_robe_shell_61.quaternion);
  }
  mesh_robe_shell_61.castShadow = options.castShadow ?? true;
  mesh_robe_shell_61.receiveShadow = options.receiveShadow ?? true;
  mesh_robe_shell_61.userData.sculptComponent = {"id": "robe-shell", "name": "Layered robe shell", "level": "macro", "role": "robe-panel", "importance": 0.86, "confidence": 0.83, "primitive": "lathe", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "root", "attachment": null, "dimensions": {"width": 0.66, "height": 1.68, "depth": 0.56, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.0, 0.0], "rotation": [0.0, 0.0, 0.0], "scale": [0.66, 1.68, 0.56]}, "actionProfile": {"animationRole": "robe-panel", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "robe-shell-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for robe-panel"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "robe-panel", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shirt", "materialLayers": ["shirt"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["deep center fold", "separated skirt silhouette", "cloth shell catches a broad key light"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["deep center fold", "separated skirt silhouette", "cloth shell catches a broad key light"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "fabric", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_robe_shell_61.add(mesh_robe_shell_61);
  meshes["robe-shell"] = mesh_robe_shell_61;
  colliders["robe-shell"] = {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["robe-panel"] ??= [];
  destructionGroups["robe-panel"].push(node_robe_shell_61);
  const socket_robe_shell_robe_shell_socket_0 = new THREE.Object3D();
  socket_robe_shell_robe_shell_socket_0.name = "robe-shell-socket";
  socket_robe_shell_robe_shell_socket_0.position.set(0.0, 0.0, 0.0);
  socket_robe_shell_robe_shell_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_robe_shell_robe_shell_socket_0.userData.socket = {"id": "robe-shell-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for robe-panel"};
  node_robe_shell_61.add(socket_robe_shell_robe_shell_socket_0);
  sockets["robe-shell:robe-shell-socket"] = socket_robe_shell_robe_shell_socket_0;

  const attachment_waist_sash_62 = {"parentSocket": "robe-waist", "localStart": [0.0, 0.6, 0.0], "localEnd": [0.0, 0.72, 0.0], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012};
  const endpoint_waist_sash_62 = makeAttachmentEndpoint(attachment_waist_sash_62);
  const node_waist_sash_62 = new THREE.Group();
  node_waist_sash_62.name = "Frost-silver waist sash__pivot";
  node_waist_sash_62.scale.set(1, 1, 1);
  if (endpoint_waist_sash_62) {
    node_waist_sash_62.position.copy(endpoint_waist_sash_62.start);
    node_waist_sash_62.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_waist_sash_62.position.set(0.0, 0.66, 0.0);
    node_waist_sash_62.rotation.set(0.0, 0.0, 0.0);
  }
  node_waist_sash_62.userData.sculptComponent = {"id": "waist-sash", "name": "Frost-silver waist sash", "level": "meso", "role": "ornament", "importance": 0.86, "confidence": 0.83, "primitive": "torus", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "robe-shell", "attachment": {"parentSocket": "robe-waist", "localStart": [0.0, 0.6, 0.0], "localEnd": [0.0, 0.72, 0.0], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.48, "height": 0.12, "depth": 0.4, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.66, 0.0], "rotation": [0.0, 0.0, 0.0], "scale": [0.48, 0.12, 0.4]}, "actionProfile": {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "waist-sash-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["wide sash seam", "silver-blue inset band"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["wide sash seam", "silver-blue inset band"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_waist_sash_62.userData.actionProfile = {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "waist-sash-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["robe-shell"] ?? root).add(node_waist_sash_62);
  nodes["waist-sash"] = node_waist_sash_62;
  const mesh_waist_sash_62Geometry = endpoint_waist_sash_62
    ? new THREE.CylinderGeometry(endpoint_waist_sash_62.endRadius, endpoint_waist_sash_62.baseRadius, endpoint_waist_sash_62.length, 16, 6)
    : new THREE.TorusGeometry(0.45, 0.08, 12, 48);
  if (!endpoint_waist_sash_62) {
    mesh_waist_sash_62Geometry.scale(0.48, 0.12, 0.4);
  }
  const mesh_waist_sash_62 = new THREE.Mesh(
    mesh_waist_sash_62Geometry,
    materialMap["base"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_waist_sash_62.name = "Frost-silver waist sash";
  if (endpoint_waist_sash_62) {
    mesh_waist_sash_62.position.copy(endpoint_waist_sash_62.midpoint);
    mesh_waist_sash_62.quaternion.copy(endpoint_waist_sash_62.quaternion);
  }
  mesh_waist_sash_62.castShadow = options.castShadow ?? true;
  mesh_waist_sash_62.receiveShadow = options.receiveShadow ?? true;
  mesh_waist_sash_62.userData.sculptComponent = {"id": "waist-sash", "name": "Frost-silver waist sash", "level": "meso", "role": "ornament", "importance": 0.86, "confidence": 0.83, "primitive": "torus", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "robe-shell", "attachment": {"parentSocket": "robe-waist", "localStart": [0.0, 0.6, 0.0], "localEnd": [0.0, 0.72, 0.0], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.48, "height": 0.12, "depth": 0.4, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.66, 0.0], "rotation": [0.0, 0.0, 0.0], "scale": [0.48, 0.12, 0.4]}, "actionProfile": {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "waist-sash-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["wide sash seam", "silver-blue inset band"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["wide sash seam", "silver-blue inset band"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_waist_sash_62.add(mesh_waist_sash_62);
  meshes["waist-sash"] = mesh_waist_sash_62;
  colliders["waist-sash"] = {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["ornament"] ??= [];
  destructionGroups["ornament"].push(node_waist_sash_62);
  const socket_waist_sash_waist_sash_socket_0 = new THREE.Object3D();
  socket_waist_sash_waist_sash_socket_0.name = "waist-sash-socket";
  socket_waist_sash_waist_sash_socket_0.position.set(0.0, 0.0, 0.0);
  socket_waist_sash_waist_sash_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_waist_sash_waist_sash_socket_0.userData.socket = {"id": "waist-sash-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"};
  node_waist_sash_62.add(socket_waist_sash_waist_sash_socket_0);
  sockets["waist-sash:waist-sash-socket"] = socket_waist_sash_waist_sash_socket_0;

  const attachment_sleeve_l_63 = {"parentSocket": "left-shoulder", "localStart": [-0.18, 0.3, 0.0], "localEnd": [-0.42, -0.34, 0.04], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012};
  const endpoint_sleeve_l_63 = makeAttachmentEndpoint(attachment_sleeve_l_63);
  const node_sleeve_l_63 = new THREE.Group();
  node_sleeve_l_63.name = "Open left sleeve__pivot";
  node_sleeve_l_63.scale.set(1, 1, 1);
  if (endpoint_sleeve_l_63) {
    node_sleeve_l_63.position.copy(endpoint_sleeve_l_63.start);
    node_sleeve_l_63.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_sleeve_l_63.position.set(-0.42, 0.34, 0.04);
    node_sleeve_l_63.rotation.set(0.0, 0.0, 0.0);
  }
  node_sleeve_l_63.userData.sculptComponent = {"id": "sleeve-l", "name": "Open left sleeve", "level": "meso", "role": "sleeve", "importance": 0.86, "confidence": 0.83, "primitive": "capsule", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "chest", "attachment": {"parentSocket": "left-shoulder", "localStart": [-0.18, 0.3, 0.0], "localEnd": [-0.42, -0.34, 0.04], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.2, "height": 0.84, "depth": 0.23, "units": "relative", "confidence": 0.83}, "transform": {"position": [-0.42, 0.34, 0.04], "rotation": [0.0, 0.0, 0.0], "scale": [0.2, 0.84, 0.23]}, "actionProfile": {"animationRole": "sleeve", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": true, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "sleeve-l-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "sleeve", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shirt", "materialLayers": ["shirt"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["open cuff", "long tapered drape", "blue inner lining"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["open cuff", "long tapered drape", "blue inner lining"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "fabric", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_sleeve_l_63.userData.actionProfile = {"animationRole": "sleeve", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": true, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "sleeve-l-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "sleeve", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["chest"] ?? root).add(node_sleeve_l_63);
  nodes["sleeve-l"] = node_sleeve_l_63;
  const mesh_sleeve_l_63Geometry = endpoint_sleeve_l_63
    ? new THREE.CylinderGeometry(endpoint_sleeve_l_63.endRadius, endpoint_sleeve_l_63.baseRadius, endpoint_sleeve_l_63.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_sleeve_l_63) {
    mesh_sleeve_l_63Geometry.scale(0.2, 0.84, 0.23);
  }
  const mesh_sleeve_l_63 = new THREE.Mesh(
    mesh_sleeve_l_63Geometry,
    materialMap["shirt"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_sleeve_l_63.name = "Open left sleeve";
  if (endpoint_sleeve_l_63) {
    mesh_sleeve_l_63.position.copy(endpoint_sleeve_l_63.midpoint);
    mesh_sleeve_l_63.quaternion.copy(endpoint_sleeve_l_63.quaternion);
  }
  mesh_sleeve_l_63.castShadow = options.castShadow ?? true;
  mesh_sleeve_l_63.receiveShadow = options.receiveShadow ?? true;
  mesh_sleeve_l_63.userData.sculptComponent = {"id": "sleeve-l", "name": "Open left sleeve", "level": "meso", "role": "sleeve", "importance": 0.86, "confidence": 0.83, "primitive": "capsule", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "chest", "attachment": {"parentSocket": "left-shoulder", "localStart": [-0.18, 0.3, 0.0], "localEnd": [-0.42, -0.34, 0.04], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.2, "height": 0.84, "depth": 0.23, "units": "relative", "confidence": 0.83}, "transform": {"position": [-0.42, 0.34, 0.04], "rotation": [0.0, 0.0, 0.0], "scale": [0.2, 0.84, 0.23]}, "actionProfile": {"animationRole": "sleeve", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": true, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "sleeve-l-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "sleeve", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shirt", "materialLayers": ["shirt"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["open cuff", "long tapered drape", "blue inner lining"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["open cuff", "long tapered drape", "blue inner lining"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "fabric", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_sleeve_l_63.add(mesh_sleeve_l_63);
  meshes["sleeve-l"] = mesh_sleeve_l_63;
  colliders["sleeve-l"] = {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["sleeve"] ??= [];
  destructionGroups["sleeve"].push(node_sleeve_l_63);
  const socket_sleeve_l_sleeve_l_socket_0 = new THREE.Object3D();
  socket_sleeve_l_sleeve_l_socket_0.name = "sleeve-l-socket";
  socket_sleeve_l_sleeve_l_socket_0.position.set(0.0, 0.0, 0.0);
  socket_sleeve_l_sleeve_l_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_sleeve_l_sleeve_l_socket_0.userData.socket = {"id": "sleeve-l-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"};
  node_sleeve_l_63.add(socket_sleeve_l_sleeve_l_socket_0);
  sockets["sleeve-l:sleeve-l-socket"] = socket_sleeve_l_sleeve_l_socket_0;

  const attachment_sleeve_r_64 = {"parentSocket": "right-shoulder", "localStart": [0.18, 0.3, 0.0], "localEnd": [0.42, -0.34, 0.04], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012};
  const endpoint_sleeve_r_64 = makeAttachmentEndpoint(attachment_sleeve_r_64);
  const node_sleeve_r_64 = new THREE.Group();
  node_sleeve_r_64.name = "Open right sleeve__pivot";
  node_sleeve_r_64.scale.set(1, 1, 1);
  if (endpoint_sleeve_r_64) {
    node_sleeve_r_64.position.copy(endpoint_sleeve_r_64.start);
    node_sleeve_r_64.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_sleeve_r_64.position.set(0.42, 0.34, 0.04);
    node_sleeve_r_64.rotation.set(0.0, 0.0, 0.0);
  }
  node_sleeve_r_64.userData.sculptComponent = {"id": "sleeve-r", "name": "Open right sleeve", "level": "meso", "role": "sleeve", "importance": 0.86, "confidence": 0.83, "primitive": "capsule", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "chest", "attachment": {"parentSocket": "right-shoulder", "localStart": [0.18, 0.3, 0.0], "localEnd": [0.42, -0.34, 0.04], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.2, "height": 0.84, "depth": 0.23, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.42, 0.34, 0.04], "rotation": [0.0, 0.0, 0.0], "scale": [0.2, 0.84, 0.23]}, "actionProfile": {"animationRole": "sleeve", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": true, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "sleeve-r-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "sleeve", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shirt", "materialLayers": ["shirt"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["open cuff", "long tapered drape", "blue inner lining"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["open cuff", "long tapered drape", "blue inner lining"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "fabric", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_sleeve_r_64.userData.actionProfile = {"animationRole": "sleeve", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": true, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "sleeve-r-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "sleeve", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["chest"] ?? root).add(node_sleeve_r_64);
  nodes["sleeve-r"] = node_sleeve_r_64;
  const mesh_sleeve_r_64Geometry = endpoint_sleeve_r_64
    ? new THREE.CylinderGeometry(endpoint_sleeve_r_64.endRadius, endpoint_sleeve_r_64.baseRadius, endpoint_sleeve_r_64.length, 16, 6)
    : buildWatertightCapsule(0.35, 0.7, 8, 16, 1);
  if (!endpoint_sleeve_r_64) {
    mesh_sleeve_r_64Geometry.scale(0.2, 0.84, 0.23);
  }
  const mesh_sleeve_r_64 = new THREE.Mesh(
    mesh_sleeve_r_64Geometry,
    materialMap["shirt"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_sleeve_r_64.name = "Open right sleeve";
  if (endpoint_sleeve_r_64) {
    mesh_sleeve_r_64.position.copy(endpoint_sleeve_r_64.midpoint);
    mesh_sleeve_r_64.quaternion.copy(endpoint_sleeve_r_64.quaternion);
  }
  mesh_sleeve_r_64.castShadow = options.castShadow ?? true;
  mesh_sleeve_r_64.receiveShadow = options.receiveShadow ?? true;
  mesh_sleeve_r_64.userData.sculptComponent = {"id": "sleeve-r", "name": "Open right sleeve", "level": "meso", "role": "sleeve", "importance": 0.86, "confidence": 0.83, "primitive": "capsule", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "chest", "attachment": {"parentSocket": "right-shoulder", "localStart": [0.18, 0.3, 0.0], "localEnd": [0.42, -0.34, 0.04], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.2, "height": 0.84, "depth": 0.23, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.42, 0.34, 0.04], "rotation": [0.0, 0.0, 0.0], "scale": [0.2, 0.84, 0.23]}, "actionProfile": {"animationRole": "sleeve", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": true, "twist": true, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "sleeve-r-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"}], "collider": {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "sleeve", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shirt", "materialLayers": ["shirt"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["open cuff", "long tapered drape", "blue inner lining"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["open cuff", "long tapered drape", "blue inner lining"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "fabric", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_sleeve_r_64.add(mesh_sleeve_r_64);
  meshes["sleeve-r"] = mesh_sleeve_r_64;
  colliders["sleeve-r"] = {"type": "capsule", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["sleeve"] ??= [];
  destructionGroups["sleeve"].push(node_sleeve_r_64);
  const socket_sleeve_r_sleeve_r_socket_0 = new THREE.Object3D();
  socket_sleeve_r_sleeve_r_socket_0.name = "sleeve-r-socket";
  socket_sleeve_r_sleeve_r_socket_0.position.set(0.0, 0.0, 0.0);
  socket_sleeve_r_sleeve_r_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_sleeve_r_sleeve_r_socket_0.userData.socket = {"id": "sleeve-r-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sleeve"};
  node_sleeve_r_64.add(socket_sleeve_r_sleeve_r_socket_0);
  sockets["sleeve-r:sleeve-r-socket"] = socket_sleeve_r_sleeve_r_socket_0;

  const attachment_shoulder_guards_65 = null;
  const endpoint_shoulder_guards_65 = makeAttachmentEndpoint(attachment_shoulder_guards_65);
  const node_shoulder_guards_65 = new THREE.Group();
  node_shoulder_guards_65.name = "Silver shoulder guards__pivot";
  node_shoulder_guards_65.scale.set(1, 1, 1);
  if (endpoint_shoulder_guards_65) {
    node_shoulder_guards_65.position.copy(endpoint_shoulder_guards_65.start);
    node_shoulder_guards_65.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_shoulder_guards_65.position.set(0.0, 0.52, 0.0);
    node_shoulder_guards_65.rotation.set(0.0, 0.0, 0.0);
  }
  node_shoulder_guards_65.userData.sculptComponent = {"id": "shoulder-guards", "name": "Silver shoulder guards", "level": "meso", "role": "armor", "importance": 0.86, "confidence": 0.83, "primitive": "sphere", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "chest", "attachment": null, "dimensions": {"width": 0.98, "height": 0.18, "depth": 0.48, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.52, 0.0], "rotation": [0.0, 0.0, 0.0], "scale": [0.98, 0.18, 0.48]}, "actionProfile": {"animationRole": "armor", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "shoulder-guards-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for armor"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "armor", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["brushed edge bevel", "cool rim highlight"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["brushed edge bevel", "cool rim highlight"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_shoulder_guards_65.userData.actionProfile = {"animationRole": "armor", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "shoulder-guards-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for armor"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "armor", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["chest"] ?? root).add(node_shoulder_guards_65);
  nodes["shoulder-guards"] = node_shoulder_guards_65;
  const mesh_shoulder_guards_65Geometry = endpoint_shoulder_guards_65
    ? new THREE.CylinderGeometry(endpoint_shoulder_guards_65.endRadius, endpoint_shoulder_guards_65.baseRadius, endpoint_shoulder_guards_65.length, 16, 6)
    : new THREE.SphereGeometry(0.5, 32, 20);
  if (!endpoint_shoulder_guards_65) {
    mesh_shoulder_guards_65Geometry.scale(0.98, 0.18, 0.48);
  }
  const mesh_shoulder_guards_65 = new THREE.Mesh(
    mesh_shoulder_guards_65Geometry,
    materialMap["base"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_shoulder_guards_65.name = "Silver shoulder guards";
  if (endpoint_shoulder_guards_65) {
    mesh_shoulder_guards_65.position.copy(endpoint_shoulder_guards_65.midpoint);
    mesh_shoulder_guards_65.quaternion.copy(endpoint_shoulder_guards_65.quaternion);
  }
  mesh_shoulder_guards_65.castShadow = options.castShadow ?? true;
  mesh_shoulder_guards_65.receiveShadow = options.receiveShadow ?? true;
  mesh_shoulder_guards_65.userData.sculptComponent = {"id": "shoulder-guards", "name": "Silver shoulder guards", "level": "meso", "role": "armor", "importance": 0.86, "confidence": 0.83, "primitive": "sphere", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "chest", "attachment": null, "dimensions": {"width": 0.98, "height": 0.18, "depth": 0.48, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.52, 0.0], "rotation": [0.0, 0.0, 0.0], "scale": [0.98, 0.18, 0.48]}, "actionProfile": {"animationRole": "armor", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [{"id": "shoulder-guards-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for armor"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "armor", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["brushed edge bevel", "cool rim highlight"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["brushed edge bevel", "cool rim highlight"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_shoulder_guards_65.add(mesh_shoulder_guards_65);
  meshes["shoulder-guards"] = mesh_shoulder_guards_65;
  colliders["shoulder-guards"] = {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": false, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["armor"] ??= [];
  destructionGroups["armor"].push(node_shoulder_guards_65);
  const socket_shoulder_guards_shoulder_guards_socket_0 = new THREE.Object3D();
  socket_shoulder_guards_shoulder_guards_socket_0.name = "shoulder-guards-socket";
  socket_shoulder_guards_shoulder_guards_socket_0.position.set(0.0, 0.0, 0.0);
  socket_shoulder_guards_shoulder_guards_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_shoulder_guards_shoulder_guards_socket_0.userData.socket = {"id": "shoulder-guards-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for armor"};
  node_shoulder_guards_65.add(socket_shoulder_guards_shoulder_guards_socket_0);
  sockets["shoulder-guards:shoulder-guards-socket"] = socket_shoulder_guards_shoulder_guards_socket_0;

  const attachment_hair_ornament_66 = {"parentSocket": "hair-crown", "localStart": [0.0, 0.05, 0.0], "localEnd": [0.0, 0.16, 0.06], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012};
  const endpoint_hair_ornament_66 = makeAttachmentEndpoint(attachment_hair_ornament_66);
  const node_hair_ornament_66 = new THREE.Group();
  node_hair_ornament_66.name = "Jade hair ornament__pivot";
  node_hair_ornament_66.scale.set(1, 1, 1);
  if (endpoint_hair_ornament_66) {
    node_hair_ornament_66.position.copy(endpoint_hair_ornament_66.start);
    node_hair_ornament_66.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_hair_ornament_66.position.set(0.0, 0.48, 0.06);
    node_hair_ornament_66.rotation.set(0.0, 0.0, 0.0);
  }
  node_hair_ornament_66.userData.sculptComponent = {"id": "hair-ornament", "name": "Jade hair ornament", "level": "micro", "role": "ornament", "importance": 0.86, "confidence": 0.83, "primitive": "torus", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "hair", "attachment": {"parentSocket": "hair-crown", "localStart": [0.0, 0.05, 0.0], "localEnd": [0.0, 0.16, 0.06], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.2, "height": 0.06, "depth": 0.2, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.48, 0.06], "rotation": [0.0, 0.0, 0.0], "scale": [0.2, 0.06, 0.2]}, "actionProfile": {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "hair-ornament-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["jade inset", "silver prongs", "small highlight catch"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["jade inset", "silver prongs", "small highlight catch"], "fidelityTier": "surface-pass", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_hair_ornament_66.userData.actionProfile = {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "hair-ornament-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["hair"] ?? root).add(node_hair_ornament_66);
  nodes["hair-ornament"] = node_hair_ornament_66;
  const mesh_hair_ornament_66Geometry = endpoint_hair_ornament_66
    ? new THREE.CylinderGeometry(endpoint_hair_ornament_66.endRadius, endpoint_hair_ornament_66.baseRadius, endpoint_hair_ornament_66.length, 16, 6)
    : new THREE.TorusGeometry(0.45, 0.08, 12, 48);
  if (!endpoint_hair_ornament_66) {
    mesh_hair_ornament_66Geometry.scale(0.2, 0.06, 0.2);
  }
  const mesh_hair_ornament_66 = new THREE.Mesh(
    mesh_hair_ornament_66Geometry,
    materialMap["base"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_hair_ornament_66.name = "Jade hair ornament";
  if (endpoint_hair_ornament_66) {
    mesh_hair_ornament_66.position.copy(endpoint_hair_ornament_66.midpoint);
    mesh_hair_ornament_66.quaternion.copy(endpoint_hair_ornament_66.quaternion);
  }
  mesh_hair_ornament_66.castShadow = options.castShadow ?? true;
  mesh_hair_ornament_66.receiveShadow = options.receiveShadow ?? true;
  mesh_hair_ornament_66.userData.sculptComponent = {"id": "hair-ornament", "name": "Jade hair ornament", "level": "micro", "role": "ornament", "importance": 0.86, "confidence": 0.83, "primitive": "torus", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "hair", "attachment": {"parentSocket": "hair-crown", "localStart": [0.0, 0.05, 0.0], "localEnd": [0.0, 0.16, 0.06], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.2, "height": 0.06, "depth": 0.2, "units": "relative", "confidence": 0.83}, "transform": {"position": [0.0, 0.48, 0.06], "rotation": [0.0, 0.0, 0.0], "scale": [0.2, 0.06, 0.2]}, "actionProfile": {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "hair-ornament-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["jade inset", "silver prongs", "small highlight catch"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["jade inset", "silver prongs", "small highlight catch"], "fidelityTier": "surface-pass", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_hair_ornament_66.add(mesh_hair_ornament_66);
  meshes["hair-ornament"] = mesh_hair_ornament_66;
  colliders["hair-ornament"] = {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["ornament"] ??= [];
  destructionGroups["ornament"].push(node_hair_ornament_66);
  const socket_hair_ornament_hair_ornament_socket_0 = new THREE.Object3D();
  socket_hair_ornament_hair_ornament_socket_0.name = "hair-ornament-socket";
  socket_hair_ornament_hair_ornament_socket_0.position.set(0.0, 0.0, 0.0);
  socket_hair_ornament_hair_ornament_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_hair_ornament_hair_ornament_socket_0.userData.socket = {"id": "hair-ornament-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"};
  node_hair_ornament_66.add(socket_hair_ornament_hair_ornament_socket_0);
  sockets["hair-ornament:hair-ornament-socket"] = socket_hair_ornament_hair_ornament_socket_0;

  const attachment_ice_sword_67 = {"parentSocket": "right-hand-socket", "localStart": [0.0, 0.0, 0.0], "localEnd": [0.0, 1.6, 0.0], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012};
  const endpoint_ice_sword_67 = makeAttachmentEndpoint(attachment_ice_sword_67);
  const node_ice_sword_67 = new THREE.Group();
  node_ice_sword_67.name = "Ice sword__pivot";
  node_ice_sword_67.scale.set(1, 1, 1);
  if (endpoint_ice_sword_67) {
    node_ice_sword_67.position.copy(endpoint_ice_sword_67.start);
    node_ice_sword_67.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_ice_sword_67.position.set(-0.46, 0.56, 0.18);
    node_ice_sword_67.rotation.set(0.0, 0.0, 0.0);
  }
  node_ice_sword_67.userData.sculptComponent = {"id": "ice-sword", "name": "Ice sword", "level": "macro", "role": "sword", "importance": 0.86, "confidence": 0.83, "primitive": "extrude", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "root", "attachment": {"parentSocket": "right-hand-socket", "localStart": [0.0, 0.0, 0.0], "localEnd": [0.0, 1.6, 0.0], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.1, "height": 1.92, "depth": 0.08, "units": "relative", "confidence": 0.83}, "transform": {"position": [-0.46, 0.56, 0.18], "rotation": [0.0, 0.0, 0.0], "scale": [0.1, 1.92, 0.08]}, "actionProfile": {"animationRole": "sword", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "ice-sword-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sword"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "sword", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["sharp ice bevel", "silver guard", "dark grip", "frost edge emissive"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["sharp ice bevel", "silver guard", "dark grip", "frost edge emissive"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_ice_sword_67.userData.actionProfile = {"animationRole": "sword", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "ice-sword-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sword"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "sword", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["root"] ?? root).add(node_ice_sword_67);
  nodes["ice-sword"] = node_ice_sword_67;
  const mesh_ice_sword_67Geometry = endpoint_ice_sword_67
    ? new THREE.CylinderGeometry(endpoint_ice_sword_67.endRadius, endpoint_ice_sword_67.baseRadius, endpoint_ice_sword_67.length, 16, 6)
    : buildExtrudeGeometry({"points": [[-0.3, -0.3], [0.3, -0.3], [0.3, 0.3], [-0.3, 0.3]], "depth": 0.1});
  if (!endpoint_ice_sword_67) {
    mesh_ice_sword_67Geometry.scale(0.1, 1.92, 0.08);
  }
  const mesh_ice_sword_67 = new THREE.Mesh(
    mesh_ice_sword_67Geometry,
    materialMap["base"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_ice_sword_67.name = "Ice sword";
  if (endpoint_ice_sword_67) {
    mesh_ice_sword_67.position.copy(endpoint_ice_sword_67.midpoint);
    mesh_ice_sword_67.quaternion.copy(endpoint_ice_sword_67.quaternion);
  }
  mesh_ice_sword_67.castShadow = options.castShadow ?? true;
  mesh_ice_sword_67.receiveShadow = options.receiveShadow ?? true;
  mesh_ice_sword_67.userData.sculptComponent = {"id": "ice-sword", "name": "Ice sword", "level": "macro", "role": "sword", "importance": 0.86, "confidence": 0.83, "primitive": "extrude", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "root", "attachment": {"parentSocket": "right-hand-socket", "localStart": [0.0, 0.0, 0.0], "localEnd": [0.0, 1.6, 0.0], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.1, "height": 1.92, "depth": 0.08, "units": "relative", "confidence": 0.83}, "transform": {"position": [-0.46, 0.56, 0.18], "rotation": [0.0, 0.0, 0.0], "scale": [0.1, 1.92, 0.08]}, "actionProfile": {"animationRole": "sword", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "ice-sword-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sword"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "sword", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["sharp ice bevel", "silver guard", "dark grip", "frost edge emissive"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["sharp ice bevel", "silver guard", "dark grip", "frost edge emissive"], "fidelityTier": "form-refinement", "colorMaterialRecipe": {"dominantAlbedo": "rgba(32, 50, 75, 1)", "secondaryAlbedo": "rgba(107, 139, 168, 1)", "materialClass": "metal", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_ice_sword_67.add(mesh_ice_sword_67);
  meshes["ice-sword"] = mesh_ice_sword_67;
  colliders["ice-sword"] = {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["sword"] ??= [];
  destructionGroups["sword"].push(node_ice_sword_67);
  const socket_ice_sword_ice_sword_socket_0 = new THREE.Object3D();
  socket_ice_sword_ice_sword_socket_0.name = "ice-sword-socket";
  socket_ice_sword_ice_sword_socket_0.position.set(0.0, 0.0, 0.0);
  socket_ice_sword_ice_sword_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_ice_sword_ice_sword_socket_0.userData.socket = {"id": "ice-sword-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for sword"};
  node_ice_sword_67.add(socket_ice_sword_ice_sword_socket_0);
  sockets["ice-sword:ice-sword-socket"] = socket_ice_sword_ice_sword_socket_0;

  const attachment_boot_engraving_68 = {"parentSocket": "boot-front", "localStart": [0.0, 0.0, 0.06], "localEnd": [0.0, 0.08, 0.08], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012};
  const endpoint_boot_engraving_68 = makeAttachmentEndpoint(attachment_boot_engraving_68);
  const node_boot_engraving_68 = new THREE.Group();
  node_boot_engraving_68.name = "Boot engraving relief__pivot";
  node_boot_engraving_68.scale.set(1, 1, 1);
  if (endpoint_boot_engraving_68) {
    node_boot_engraving_68.position.copy(endpoint_boot_engraving_68.start);
    node_boot_engraving_68.rotation.set(0.0, 0.0, 0.0);
  } else {
    node_boot_engraving_68.position.set(-0.02, 0.02, 0.08);
    node_boot_engraving_68.rotation.set(0.0, 0.0, 0.0);
  }
  node_boot_engraving_68.userData.sculptComponent = {"id": "boot-engraving", "name": "Boot engraving relief", "level": "micro", "role": "ornament", "importance": 0.86, "confidence": 0.83, "primitive": "box", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "foot-l", "attachment": {"parentSocket": "boot-front", "localStart": [0.0, 0.0, 0.06], "localEnd": [0.0, 0.08, 0.08], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.16, "height": 0.24, "depth": 0.03, "units": "relative", "confidence": 0.83}, "transform": {"position": [-0.02, 0.02, 0.08], "rotation": [0.0, 0.0, 0.0], "scale": [0.16, 0.24, 0.03]}, "actionProfile": {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "boot-engraving-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shoes", "materialLayers": ["shoes"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["blue engraved linework", "raised silver edge"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["blue engraved linework", "raised silver edge"], "fidelityTier": "surface-pass", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "unknown", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_boot_engraving_68.userData.actionProfile = {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "boot-engraving-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}};
  (nodes["foot-l"] ?? root).add(node_boot_engraving_68);
  nodes["boot-engraving"] = node_boot_engraving_68;
  const mesh_boot_engraving_68Geometry = endpoint_boot_engraving_68
    ? new THREE.CylinderGeometry(endpoint_boot_engraving_68.endRadius, endpoint_boot_engraving_68.baseRadius, endpoint_boot_engraving_68.length, 16, 6)
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_boot_engraving_68) {
    mesh_boot_engraving_68Geometry.scale(0.16, 0.24, 0.03);
  }
  const mesh_boot_engraving_68 = new THREE.Mesh(
    mesh_boot_engraving_68Geometry,
    materialMap["shoes"] ?? new THREE.MeshStandardMaterial({ color: 0x888888 })
  );
  mesh_boot_engraving_68.name = "Boot engraving relief";
  if (endpoint_boot_engraving_68) {
    mesh_boot_engraving_68.position.copy(endpoint_boot_engraving_68.midpoint);
    mesh_boot_engraving_68.quaternion.copy(endpoint_boot_engraving_68.quaternion);
  }
  mesh_boot_engraving_68.castShadow = options.castShadow ?? true;
  mesh_boot_engraving_68.receiveShadow = options.receiveShadow ?? true;
  mesh_boot_engraving_68.userData.sculptComponent = {"id": "boot-engraving", "name": "Boot engraving relief", "level": "micro", "role": "ornament", "importance": 0.86, "confidence": 0.83, "primitive": "box", "topologyClass": "assembled-solid", "topologyRationale": "The visible part has a bounded solid envelope and is assembled at a named seam or socket.", "parent": "foot-l", "attachment": {"parentSocket": "boot-front", "localStart": [0.0, 0.0, 0.06], "localEnd": [0.0, 0.08, 0.08], "contactType": "overlap-and-seam", "embedDepth": 0.018, "overlap": 0.024, "gapTolerance": 0.012}, "dimensions": {"width": 0.16, "height": 0.24, "depth": 0.03, "units": "relative", "confidence": 0.83}, "transform": {"position": [-0.02, 0.02, 0.08], "rotation": [0.0, 0.0, 0.0], "scale": [0.16, 0.24, 0.03]}, "actionProfile": {"animationRole": "ornament", "pivot": {"mode": "center", "localPosition": [0.0, 0.0, 0.0], "axis": [0.0, 1.0, 0.0], "confidence": 0.84}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": true, "visibility": true, "materialState": true}, "sockets": [{"id": "boot-engraving-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"}], "collider": {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"}, "constraints": [], "destruction": {"breakable": true, "fractureGroup": "ornament", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "hidden"}}, "material": "shoes", "materialLayers": ["shoes"], "deformations": [], "joints": [], "seams": [], "localFeatures": ["blue engraved linework", "raised silver edge"], "surfaceDetail": {"macroRoughness": 0.28, "microRoughness": 0.16, "bumpAmplitude": 0.08, "normalPattern": "imagegen-reference-derived weave or engraved relief", "displacementPattern": "none; preserve the gameplay silhouette", "occlusionPattern": "contact-darkened seams and overlaps", "edgeWearPattern": "subtle silver edge catch on exposed trim", "notes": "Keep the detail visible under the night grade without emissive washout."}, "evidenceRefs": ["full-object", "costume-close"], "details": ["blue engraved linework", "raised silver edge"], "fidelityTier": "surface-pass", "colorMaterialRecipe": {"dominantAlbedo": "rgba(239, 245, 255, 1)", "secondaryAlbedo": "rgba(126, 167, 200, 1)", "materialClass": "unknown", "materialClassConfidence": 0.84, "colorGradient": {"type": "linear", "stops": [{"position": 0.0, "color": "rgba(247, 250, 255, 1)"}, {"position": 1.0, "color": "rgba(108, 143, 182, 1)"}]}, "evidenceRefs": ["costume-close"]}};
  node_boot_engraving_68.add(mesh_boot_engraving_68);
  meshes["boot-engraving"] = mesh_boot_engraving_68;
  colliders["boot-engraving"] = {"type": "box", "offset": [0.0, 0.0, 0.0], "scale": [1.0, 1.0, 1.0], "isTrigger": true, "notes": "lightweight gameplay proxy; render mesh remains independent"};
  destructionGroups["ornament"] ??= [];
  destructionGroups["ornament"].push(node_boot_engraving_68);
  const socket_boot_engraving_boot_engraving_socket_0 = new THREE.Object3D();
  socket_boot_engraving_boot_engraving_socket_0.name = "boot-engraving-socket";
  socket_boot_engraving_boot_engraving_socket_0.position.set(0.0, 0.0, 0.0);
  socket_boot_engraving_boot_engraving_socket_0.rotation.set(0.0, 0.0, 0.0);
  socket_boot_engraving_boot_engraving_socket_0.userData.socket = {"id": "boot-engraving-socket", "localPosition": [0.0, 0.0, 0.0], "localRotation": [0.0, 0.0, 0.0], "purpose": "attachment socket for ornament"};
  node_boot_engraving_68.add(socket_boot_engraving_boot_engraving_socket_0);
  sockets["boot-engraving:boot-engraving-socket"] = socket_boot_engraving_boot_engraving_socket_0;
...
export function createSeolryeongFrostSilkSwordswomanLookDevLights(
  mode: 'neutral' | 'grazing' | 'reference' = 'neutral',
): THREE.Group {
  const lights = new THREE.Group();
  lights.name = "Seolryeong frost-silk swordswoman look-dev lights";
  ...
  lights.userData.lightingFromPhoto = [{"type": "key", "direction": "high front-left", "color": "cool moon blue", "intensity": 1.0, "evidenceRef": "full-object"}, {"type": "fill", "direction": "camera-side low fill", "color": "desaturated cyan", "intensity": 0.34, "evidenceRef": "full-object"}, {"type": "rim", "direction": "back-right edge", "color": "silver cyan", "intensity": 0.72, "evidenceRef": "costume-close"}, {"type": "exposure", "value": 0.84, "toneMapping": "ACES filmic", "background": "deep blue neutral", "evidenceRef": "full-object"}, {"type": "contact shadow", "behavior": "soft ground shadow under boots and robe hem", "ambientOcclusion": true, "evidenceRef": "lower-silhouette"}];
...
export function createSeolryeongFrostSilkSwordswomanEnvironment(renderer: THREE.WebGLRenderer): THREE.Texture {
...
export function frameSeolryeongFrostSilkSwordswomanCamera(
...
export function createSeolryeongFrostSilkSwordswomanPresentationComposer(
...
export function configureSeolryeongFrostSilkSwordswomanRenderer(renderer: THREE.WebGLRenderer): void {
...
export function createSeolryeongFrostSilkSwordswomanInspectControls(
  camera: THREE.Camera,

...
function isDomControlTarget(target) {
  const element = target?.closest?.('button,a,input,select,textarea,[role="button"]') ?? target
  const tag = String(element?.tagName ?? element?.nodeName ?? '').toLowerCase()
  return tag === 'button' || tag === 'a' || tag === 'input' || tag === 'select' ||
    tag === 'textarea' || element?.getAttribute?.('role') === 'button'
}
...
export class Input {
  ...
  constructor(target = window, pads = () => (typeof navigator === 'undefined' ? [] : navigator.getGamepads?.() ?? [])) {
    ...
    this._onKeyDown = (e) => {
      if (e.repeat) return
      // A visible DOM control owns its native activation. If this window-level
      // listener also latches Enter/Space, the game loop would invoke the same
      // callback once more after the browser dispatches the button click.
      if (isDomControlTarget(e.target) && (e.code === 'Enter' || e.code === 'Space')) return
      if (e.code in MOVE_KEYS || e.code === 'Space') e.preventDefault()
      this.down.add(e.code)
      if (e.code === 'Space') {
        this._dash = true
        // Space is dash during combat and confirm in menus. The state-specific
        // consumers decide which latch is meaningful; both remain edge-triggered.
        this._confirm = true
      }
      else if (e.code === 'KeyP' || e.code === 'Escape') this._pause = true
      else if (e.code === 'Enter') this._confirm = true
      else if (e.code === 'KeyE') this._interact = true
  ...
  poll() {
    ...
    if (!pad) {
      // A disconnected pad must not leave its last vector latched. Recompute
      // here so a held keyboard key takes ownership on the same frame.
      this._padX = 0
      this._padZ = 0
      this._recompute()
      this.source = 'keyboard'
    ...
    if (mag > DEADZONE) {
      ...
      this._padX = 0
      this._padZ = 0
      // A neutral stick is not a new input source. If a keyboard direction is
      // still held, hand movement back to it immediately instead of freezing
      // the player at a zero gamepad vector.
      this._recompute()
    }
    ...
    if (pressed(PAD.west)) this._interact = true
  ...
  /**
   * Discard a dash edge that was generated by a menu-confirm button.
   *
   * Space and the controller south face intentionally feed both the combat
   * dash and menu-confirm latches. A modal may transition back to `playing`
   * during the same frame, so the game loop needs an explicit way to consume
   * the non-applicable dash edge without treating it as gameplay input.
   */
  discardDash() { this._dash = false }

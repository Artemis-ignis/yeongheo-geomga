# Void-Iron Scale Sentinel v2

이 폴더는 영허검가의 근접 `demonCultivator` 모델에 실제로 연결된 하나의 기준 자산과 검수 증거만 보관합니다. 새 이미지를 계속 쌓기 위한 폴더가 아닙니다.
...
4. isolated reference의 공식 admission은 통과했고, PBR 추출은 confidence `0.86`으로 보고됐습니다. AO, height, normal, roughness는 `public/assets/materials/img2three/`에 연결했습니다.
...
- 기준 이미지: `public/assets/characters/void-iron-scale-sentinel-reference-v2.png`
- ImageGen 표면 타일: `public/assets/materials/guardians/void-iron-scale-armor-v1.png`
- 공식 img2threejs 채널: `public/assets/materials/img2three/void-iron-scale_{ao,height,normal,roughness}.png`
- 실제 모델 코드: `src/art/NearEnemyModels.js`의 `buildDemonCultivator()`
- 근접 LOD 삽입: `src/entities/EnemyManager.js`의 near-detail 슬롯
- 실제 형태 검수: `near-sheet.html` + `src/dev/nearModelSheet.js`

## 현재 품질 경계

현재 결과는 기존 도형 덩어리보다 분명히 개선된 근접 적 모델이지만, 스키닝된 고품질 GLB/GLTF나 AAA급 완성품은 아닙니다. 단일 이미지에서 보이지 않는 뒷면과 관절을 임의로 확정할 수 없고, img2threejs 자체도 코드 기반 procedural reconstruction 도구이기 때문입니다. 실제 검수에서 아직 둥근 관절판과 단순화된 갑주가 기준 이미지보다 단순하다는 점을 확인했습니다. 따라서 이 자산의 상태는 `near-detail refinement / not AAA`로 기록합니다.

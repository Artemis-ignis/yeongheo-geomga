...
- 늑대 focused 장면은 실루엣 검사용 제어 장면이며 위 `4be14c6` 무치트 7분 전체 런과 역할을 섞지 않습니다. 코드상 `ashRaven`이 여전히 `yorang` 늑대 아틀라스를 재사용하지만 현재 release stage 목록은 `jade` 하나이므로, 이는 공개 첫 런 P0가 아니라 잠긴 `ember` 비경의 P1 콘텐츠 결함입니다. 전체 commercial visual gate와 release 승인은 다른 미해결 항목 때문에 계속 FAIL입니다.

...
describe('runtime asset rights audit CLI', () => {
  it('reports the current 0/76 gate as BLOCKED with a nonzero exit', () => {
    const result = runCli()

    expect(result.status).toBe(1)
    expect(result.stdout).toContain('RIGHTS AUDIT: BLOCKED')
    expect(result.stdout).toContain('rights cleared: 0/76')
    expect(result.stdout).toContain('blocked: 76/76')
  ...
  it('writes JSON only for an explicit output path and includes every asset field', () => {
    ...
    expect(report.counts).toMatchObject({ total: 76, cleared: 0, blocked: 76 })
    ...
    expect(report.assets).toHaveLength(76)
  ...
  it('uses the current 76-row ledger section instead of historical fingerprint tables', () => {
    const report = buildRightsReport()

    expect(report.inputs.ledgerRowCount).toBe(76)

...
Windows에서는 루트의 `영허검가 실행.vbs`를 더블클릭합니다. 실행기는 최소화된 상태로 로컬 서버를 유지하며 브라우저를 한 번만 엽니다. Windows Script Host가 차단된 환경에서는 `게임시작.bat`를 예비 경로로 사용합니다. 개발 환경은 Node.js 20.19 이상을 사용합니다.

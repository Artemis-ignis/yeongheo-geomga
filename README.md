...
Windows에서는 루트의 `영허검가 실행.vbs`만 더블클릭합니다. 로컬 `dist`가 있으면 최소화된 정적 서버로 그 빌드를 열고, GitHub에서 새로 받은 소스처럼 `dist`가 없으면 설치 없이 최신 공개 빌드 `https://yeongheo-geomga.vercel.app/`를 엽니다. 두 경로 모두 브라우저는 한 번만 열립니다. 개발 환경은 Node.js 20.19 이상을 사용합니다.
